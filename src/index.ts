...
const server = new McpServer(
  { name: "chinese-almanac-mcp", version: PKG_VERSION },
  {
    instructions:
...
// ─── shared fixed-day + patron logic (ported & cross-validated) ──

interface DayRec { [k: string]: unknown }
...
interface VetoCheck {
  vetoes: string[];          // zh+en veto labels
  adjustments: { zh: string; en: string; points: number }[];
  fuyin: boolean;
  localAdjustment: number;
}

function checkFixedDaysAndPatron(day: DayRec, patronSlug: string | null, vetoSanniang: boolean): VetoCheck {
  const vetoes: string[] = [];
  const adjustments: VetoCheck["adjustments"] = [];
  let fuyin = false;
  let localAdjustment = 0;

  const lunar = (day.lunar ?? {}) as Record<string, unknown>;
  const lm = Number(lunar.month ?? 0);
  const ld = Number(lunar.day ?? 0);
  const pillar = (day.day_pillar ?? {}) as Record<string, unknown>;
  const gzI = gzIndex(String(pillar.stem_cn ?? ""), String(pillar.branch_cn ?? ""));

  if (isYangGongJi(lm, ld)) vetoes.push("杨公忌 Yang Gong Ji");
  if (isSanniangSha(ld, vetoSanniang)) vetoes.push("三娘煞 Sanniang Sha");
  if (isShiEDaBai(gzI)) vetoes.push("十恶大败 Shi E Da Bai");

  if (patronSlug) {
    const relations = (day.relations ?? {}) as Record<string, unknown>;
    const map = Array.isArray(relations.map) ? relations.map : [];
    const relEntry = map.find((m) => (m as Record<string, unknown>).slug === patronSlug) as Record<string, unknown> | undefined;
...
async function enrichTopHours(dateStr: string, patronSlug: string | null, limit = 3) {
  const d = await api("hours", { date: dateStr });
  const hours = Array.isArray(d.hours) ? d.hours : [];
  return hours
    .filter((h) => {
      const rec = h as Record<string, unknown>;
      const hh = (rec.huanghei ?? {}) as Record<string, unknown>;
      if (hh.type !== "yellow") return false;
      if (patronSlug && String(rec.clash_zodiac ?? "").toLowerCase() === patronSlug) return false;
      return true;
    })
    .slice(0, limit);
}
...
server.tool(
  ...
  async ({ activity, days, weekend_only, patron_birth }) => {
    ...
    const results: DayRec[] = [];
    for (const rec of recs) {
      const r = rec as Record<string, unknown>;
      const dateStr = String(r.date ?? "");
      if (!dateStr) continue;
      const day = await api("day", { date: dateStr });
      if (!day || !day.lunar) continue;

      const base = Number(r.score ?? 0);
      const { vetoes, adjustments, fuyin, localAdjustment } =
        checkFixedDaysAndPatron(day, patronSlug, profile.vetoSanniang);
      if (vetoes.length > 0) {
      ...
      const lunar = day.lunar as Record<string, unknown>;
      const pillar = day.day_pillar as Record<string, unknown>;
      const officer = day.day_officer_zhi_shen as Record<string, unknown>;
      const belt = day.belt as Record<string, unknown>;
      const jx = (day.jishen_xiongsha ?? {}) as Record<string, unknown>;
      results.push({
        date: dateStr,
        rejected: false,
        engine_score: base,
        ...
        nayin: (day.folk as Record<string, unknown> | undefined)?.nayin ?? [],
        fuyin,
        adjustments,
...
server.tool(
  "pick_dates_deep",
  "Deep day-by-day scan for activities without an engine shortlist 深度逐日择日 — burial 安葬/funerals and ancestor worship 祭祀/祭拜. Iterates every day in the window (max 31 days): hard-vetoes fixed inauspicious days and patron clash, scores Day Officer + Yellow/Black Belt + auspicious gods + Yi-list match. Slower (1 request/day, cached server-side by the API) — keep windows short.",
  {
  ...
  async ({ activity, start, end, patron_birth, top }) => {
    ...
    if (span > 31) return fmt({ error: "window too long", tip: "deep scan is capped at 31 days" });

    const patronSlug = patron_birth ? zodiacFromBirthDate(patron_birth) : null;
    const deepYi: Record<string, string[]> = {
      burial: ["burial", "funeral", "安葬", "启钻"],
      "ancestor-worship": ["worship", "prayer", "ancestor", "祭祀"],
    };
    const goodOfficers: Record<string, string[]> = {
      burial: ["Close", "Remove", "Stable", "Collect"],
      "ancestor-worship": ["Stable", "Close", "Remove", "Full"],
    };
    const preferGods: Record<string, string[]> = {
    ...
    const out: DayRec[] = [];
    const termsCache = new Map<number, Record<string, { date?: string }>>();
    ...
    while (cursor <= endD && out.length < (top ?? 5) * 4) {
      ...
      if (day && day.lunar && !day._error) {
        ...
        if (!termsCache.has(yr)) {
          ...
          termsCache.set(yr, (t.terms ?? {}) as Record<string, { date?: string }>);
        }
        let veto = "";
        if (isYangGongJi(lm, ld)) veto = "杨公忌";
        if (!veto && isShiEDaBai(gzI)) veto = "十恶大败";
        if (!veto) {
        ...
        if (!veto && patronSlug) {
          ...
          const relEntry = map.find((m) => (m as Record<string, unknown>).slug === patronSlug) as Record<string, unknown> | undefined;
          const verdict = patronRelation(relEntry ? String(relEntry.rel) : "plain");
          if (verdict.action === "veto") veto = `本命${REL_CN[verdict.rel]}`;
        }
        if (!veto) {
          const officerEn = String((day.day_officer_zhi_shen as Record<string, unknown>).en ?? "");
          const beltType = String((day.belt as Record<string, unknown>).type ?? "");
          let score = OFFICER_SCORE[officerEn] ?? 0;
          if (beltType === "yellow") score += 20;
          else if (beltType === "black") score -= 5;
          const reasons: string[] = [];
          if (OFFICER_SCORE[officerEn]) { reasons.push(`建除【${(day.day_officer_zhi_shen as Record<string, unknown>).cn}】${officerEn} (${OFFICER_SCORE[officerEn] >= 0 ? "+" : ""}${OFFICER_SCORE[officerEn]})`); }
          reasons.push(`值神${(day.belt as Record<string, unknown>).name_cn} ${beltType} belt (${beltType === "yellow" ? "+20" : "-5"})`);
          const jx = (day.jishen_xiongsha ?? {}) as Record<string, unknown>;
          const godsA = Array.isArray(jx.auspicious) ? (jx.auspicious as string[]) : [];
...
server.tool(
  "list_activities",
  "List the supported event types with all accepted synonyms (EN/CN) 列出支持的择日活动与全部中英同义词 — use this to validate the activity argument. 8 fast events use pick_auspicious_dates; burial 安葬 & ancestor-worship 祭祀 use pick_dates_deep.",
  {},
  async () => fmt({
    fast_events: [
